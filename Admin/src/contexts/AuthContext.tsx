...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchProfile = async (userId: string) => {
    try {
      ...
      if (data as any) {
        // Get user permissions - using type assertion since function may not exist in admin DB
        const { data: permissions } = (await supabase
          .rpc('get_user_permissions', { user_id: userId })) as any;
        ...
        setIsBlocked((data as any).blocked || false);
        return userProfile;
      }
    } catch (error) {
      console.error('Error fetching profile:', error);
  ...
  const checkIfUserBlocked = async (userId: string) => {
    try {
      ...
      return (data as any) && (data as any).blocked ? (data as any).blocked : false;
    } catch (error) {
      console.error('Error checking blocked status:', error);
      return false;
    }
